...
use crate::model::game_state::{Piece, Position, Move};
...
impl Display for UciMove {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::result::Result<(), std::fmt::Error> {
        let from_file_repr = std::char::from_u32(('a' as u32) + (self.0.file() as u32 - 1)).unwrap();
        ...
        let promotes_repr = self.2.map(|piece| match piece {
            Piece::QUEEN => "q",
            Piece::ROOK => "r",
            Piece::BISHOP => "b",
            Piece::KNIGHT => "n",
        ...
        write!(f, "{}{}{}{}{}", from_file_repr, from_rank_repr, to_file_repr, to_rank_repr, promotes_repr)
    }
} 
...
pub fn parse_move(uci_move: &str) -> Result<UciMove, String> {
    ...
    let from = parse_position(uci_move.chars().nth(0).unwrap(), uci_move.chars().nth(1).unwrap());
    let to = parse_position(uci_move.chars().nth(2).unwrap(), uci_move.chars().nth(3).unwrap());
